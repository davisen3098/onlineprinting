...
if (isset($_POST['confirm'])) {
    $sql = "update orders set o_status =:o_status where o_id =:o_id";
    $stmt = $db->prepare($sql);
    $params = [
        'o_id' => $_POST['o_id'],
        'o_status' => 1
    ...
    $db_test = new DB_con();
    $getUser = $db_test->getUserDetails($_POST['cust_id']);
    $stmt->execute($params);
    $pay = $db_test->addPay($_POST['o_id']);


    $mailStatus = sendOrderMail($getUser['cust_email']);
                                        ...
                                        <tbody>
                                            ...
                                            ?>
                                                    <tr>
                                                        ...
                                                        <td><?= $order['cust_id']; ?></td>
                                                        <td><?php
                                                            $date = date('d-m-Y', strtotime($order['o_date']));
                                                            echo $date;
                                                            ?></td>
                                                        ...
                                                        <?php } ?>
                                                        <td>Rs <?= $order['o_total']; ?></td>

                                                        <td>
                                                            <form action="" method="post" class="d-inline">
                                                                <input type="hidden" name="o_id" value="<?= $order['o_id'] ?>">
                                                                <input type="hidden" name="cust_id" value="<?= $order['cust_id'] ?>">


                                                                <?php if ($order['o_status'] == 0) {

                                                                ?>
                                                                    <button type="submit" name="confirm" value="<?= $order['o_id'] ?>" class="btn btn-success btn-sm">Confirm</button>
                                                                <?php } ?>
