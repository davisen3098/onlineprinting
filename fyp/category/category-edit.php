...
if (isset($_GET['id'])) {
    $cat_id = $_GET['id'];
    $dbh = new DB_con();
    $query_run = $dbh->getCatById($cat_id);
...
if (isset($_POST['update_category'])) {
    $result = $update_db->updateCat($cat_id, $_POST['cat_name'], $_POST['cat_desc'], $_POST['pt_dropdown']);
    if ($result) {
        echo "<script>alert('Categoty updated successfully')</script>";
    } else {
                        ...
                        <div class="col-md-8">
                            <form action="" method="post">
                                <div class="form-group">
                                    <input type="text" class="form-control" id="cat_name" name="cat_name" placeholder="Category name" value="<?= $cat_name ?>">
                                </div>
                                <div class="form-group mt-3">
                                    <textarea placeholder="Description" id="cat_desc" name="cat_desc" class="form-control"><?= $cat_desc ?></textarea>
                                </div>
                                <select id="pt_dropdown" name="pt_dropdown" class="form-control mt-3">
                                    <option value="0"> Select product type </option>
                                    <?php

                                    $fetchCat = new DB_con();
                                    $result = $fetchCat->productTypeDropdown();
                                    while ($row = mysqli_fetch_assoc($result)) {
                                        $pt_name = $row['pt_name'];
                                        $prt_id = $row['pt_id'];

                                        echo  "<option value='" . $prt_id . "' >" . $pt_name . "</option>";
                                    }

                                    ?>
                                </select>
                                <div class="form-group mt-3">
                                    <input type="submit" class="btn btn-success form-control" value="Update category" name="update_category" />
                                </div>
                            </form>
                            </form>
