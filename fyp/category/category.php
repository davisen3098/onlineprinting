...
if (isset($_POST['submit'])) {
    if (isset($_POST['cat_name'], $_POST['cat_desc'], $_POST['pt_dropdown']) && !empty($_POST['cat_name']) && !empty($_POST['cat_desc']) && !empty($_POST['pt_dropdown'])) {
        $regex = '/^[a-zA-Z\s]*$/';
        if (preg_match($regex, $_POST['cat_desc']) && preg_match($regex, $_POST['cat_name'])) {
            $db = new DB_con();
            $cat_name = $_POST['cat_name'];
            $cat_desc = $_POST['cat_desc'];
            $pt_id = $_POST['pt_dropdown'];

            $result = $db->addCategory($cat_name, $cat_desc, $pt_id);
            ...
            echo "<script> alert('Only letters are allowed') </script>";
        }
    }
}
                        ...
                        <div class="col-md-8">
                            <form action="" method="post">
                                ...
                                <div class="form-group mt-3">
                                    <textarea placeholder="Description" id="cat_desc" name="cat_desc" class="form-control"></textarea>
                                </div>
                                ...
                                <div class="form-group mt-3">
                                    <input type="submit" class="btn btn-success form-control" value="Add category" name="submit" />
                                </div>
                            </form>
                        </div>
                    </div>
